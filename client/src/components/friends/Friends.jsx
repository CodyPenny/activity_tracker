...
class Friends extends Component {
  ...
  componentDidMount = async () => {
    let friend_promises = this.props.friends.map((friend) => {
      if (friend !== this.props.user) {
        return getFriend(friend);
      }
    });

    friend_promises = friend_promises.filter((promise) => promise !== undefined);

    let friends = await Promise.all(friend_promises);
